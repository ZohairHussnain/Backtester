...
- `ibkr_sync.py` — read-only pull of live broker state (open orders/positions/account/fills). `--apply --confirm` rebuilds `portfolio_state.paper.json` from broker truth. This is the supported way to adopt broker positions the local ledger never recorded — whether opened on **another machine** sharing the paper account or by a **manual buy/sell in TWS**. (`--reconcile-only` can't adopt them: it matches fills ids-only by `perm_id`/`broker_order_id`, so a fill with no local order is skipped by design — the same guard that keeps stale `ib.fills()` history out of fresh state. Don't loosen it.) Adopted broker-only positions arrive dated at the sync time with zero stop/target; follow with `--reconcile-only` to backfill exit levels. Never submits orders.
...
- **Reconciliation is idempotent + crash-safe.** Fills match on stable `perm_id`; processed `execId`s live in portfolio state (`processed_fill_ids`) and commit atomically with cash/positions; `fills.csv` is an audit log written only after the save. Re-running `--reconcile-only` never double-counts. After applying fills, `update_lifecycle_statuses` writes terminal statuses back to `orders_lifecycle.csv` (matched→`FILLED`; prior-day unfilled→`CANCELLED`; same-day open left `SUBMITTED`). This is **conservative by design** — it never terminalizes a same-day unfilled order, so it can't strand a not-yet-arrived fill (a fill only matches orders still loaded as pending). Audit-only; it never raises.
...
- **Exit decisioning mirrors the backtest.** Every BUY fill stamps `stop_price = entry*(1-STOP_LOSS_PCT)` and `target_price = entry*(1+TARGET_PROFIT_PCT)` off the (blended) fill price (`apply_buy_fill`); `backfill_exit_levels` retrofits any legacy position stored with zeroed levels. `run_daily.determine_exits` is the single exit authority: it scans completed daily bars **strictly after** the entry date (D+1 onward — never the entry bar), flags `stop_hit`/`target_hit` on `low<=stop`/`high>=target`, breaks a same-bar both-hit tie by open-distance (stop wins ties, matching `LabelEngine.compute_target_stop`), and falls back to `max_hold_exit` at `MAX_HOLD_DAYS` counted in **NYSE trading sessions** (`trading_days_between`, not calendar days) to match the backtest's bar-based horizon. Exits act at the next open (MOO/MKT SELL). The exit reason flows through `order_generator` into the SELL order, and `apply_fill` maps the pending SELL order's reason into `apply_sell_fill` so the realized `trade_history` record carries `stop_hit`/`target_hit`/`max_hold_exit` (falls back to `ibkr_fill` for a manual/unmatched sell). Keep this aligned with `LabelEngine`.
- **Rotation/replacement exit (off by default).** When `ROTATION_ENABLED`, `OrderGenerator` may replace the weakest current holding (ranked by **today's prediction probability** — never "AUC", a model-level metric) with a materially stronger candidate (`p_new >= p_weak + ROTATION_MIN_PROB_IMPROVEMENT`). Holdings already exiting that day, and holdings with no prediction row today, are excluded from the weakest pick. It is **atomic** — the `rotation_exit` SELL is emitted only if the paired BUY can be built as ≥1 whole share (no naked sell) — and capped at `ROTATION_MAX_PER_DAY`. Positions don't store entry probability; "strength" is recomputed from the day's predictions. All tunables live in `config.py`.
- **Reconciliation applies SELLs before BUYs.** `run_reconcile_only` and the `ibkr_paper` MKT path sort fills SELL-first so a same-day rotation's proceeds are booked before the paired BUY consumes them (local cash changes only from fills).
