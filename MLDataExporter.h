...
public:
	static size_t save_csv(
		...
		std::unordered_map<std::string, int> labels_by_key;
		labels_by_key.reserve(labels.size());
		for (const auto& label : labels) {
			labels_by_key[key(label.date, label.ticker)] = label.label;
		}
		...
		file << "date,ticker,ret_1d,ret_5d,ret_10d,ret_20d,rsi_14,atr_14_pct,volume_ratio_20,dist_ma20,dist_ma50,dist_ma200,rolling_vol_20,label\n";
